...
% Last Revision: Friday 29/01/2016 22:51.

function script_faster_rcnn_kitti_ZFHalf()
...
%     run(fullfile(fileparts(fileparts(mfilename('fullpath'))), 'startup'));
    ...
    opts.do_val = true;
    model = Model.ZF_for_Faster_RCNN_VOC0712_jingjing;
    % cache base
    cache_base_proposal = 'faster_rcnn_Kitti_ZF';
    cache_base_fast_rcnn = '';

    %% ----------------- Load Training DB ----------------
    db_name = '/media/hanxi/m2_SSD/Kitti/voc_format_dataset/kitti_voc_data_r80.mat';
    dataset = load(db_name, 'fake_data');
    dataset = dataset.fake_data;

    %% -------------------- TRAIN --------------------
    % conf
    conf_proposal = proposal_config('image_means', ...
                model.mean_image, 'feat_stride', model.feat_stride);
    ...
    [conf_proposal.anchors, conf_proposal.output_width_map, ...
        conf_proposal.output_height_map] = proposal_prepare_anchors(conf_proposal, ...
                    model.stage1_rpn.cache_name, model.stage1_rpn.test_net_def_file);
    ...
    % test
    dataset.roidb_train = cellfun(@(x, y) Faster_RCNN_Train.do_proposal_test(conf_proposal, ...
        model.stage1_rpn, x, y), dataset.imdb_train, dataset.roidb_train, 'UniformOutput', false);
    ...
    % test
    dataset.roidb_train = cellfun(@(x, y) Faster_RCNN_Train.do_proposal_test(conf_proposal, ...
        model.stage2_rpn, x, y), dataset.imdb_train, dataset.roidb_train, 'UniformOutput', false);
    dataset.roidb_test = Faster_RCNN_Train.do_proposal_test(conf_proposal, ...
